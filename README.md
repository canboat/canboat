...
> [!WARNING]
> ## ON AN iKONVERT, THE TRANSMIT LIST IS FINAL
>
> **Once you name any transmit PGN (`--tx-pgn`, `--ikonvert-tx`, or
> `pgn_lists.tx`), canboat REFUSES TO SEND EVERY PGN THAT IS NOT ON THE
> LIST.** The gateway only transmits the PGNs in its list, and that list can
> only be set before the gateway goes on the bus, so canboat drops the rest
> up front, logging each refused PGN once, instead of handing the gateway
> frames it will reject.
>
