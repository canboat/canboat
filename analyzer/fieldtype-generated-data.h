...
FieldType fieldTypeList[] = {
    ...
    {.name = "DYNAMIC_FIELD_KEY",
     ...
     .comment = "These values have been determined by reverse engineering, given the known values it is anticipated that there are unknown enumeration values and some known values have incorrect datatypes",
     .hasSign = False,
     .sentinels = SENTINEL_TOP_OF_RANGE,
     .pf = fieldPrintLookup},
    ...
    {.name = "STRING_FIX",
     ...
     .comment = "The character encoding is not defined by NMEA 2000. Bytes below 0x80 are ASCII, but above that devices differ, so CANboat decodes the content as UTF-8 when the bytes form well-formed UTF-8 and as ISO 8859-1 (Latin-1) otherwise. See the STRING_LAU comment for the observations behind this.",
    ...
    {.name = "STRING_LAU",
     ...
     .comment = "Observed on a single bus: a Fusion sends UTF-8 under control byte 1 (bytes C5 AB for U+016B, see samples/fusion-126998-utf8.raw) while a B&G sends ISO 8859-1 (byte E6 for U+00E6, see samples/bandg-129285-latin1.raw) -- the same field type under the same control byte. The encoding therefore follows from neither the field type nor the control byte, and CANboat decodes the content as UTF-8 when the bytes form well-formed UTF-8 and as ISO 8859-1 (Latin-1) otherwise. The two devices also frame the field differently: the B&G appends a zero byte and counts it in the length, the Fusion does neither. Control byte 0 has so far only been seen on fields that were empty.",
     .variableSize = True,
     .sentinels = SENTINEL_EMPTY_STRING,
