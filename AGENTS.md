...
> under `docs/` that is a database artifact, `docs/canboat.dbc`, the
> analyzer's data tables (`analyzer/lookup-generated-data.h`, `analyzer/*-data.h`)
> AND the Rust tables (`crates/canboat/src/engine/schema_generated.rs`,
> `crates/canboat/src/io/fastpacket_generated.rs`) are
...
>
> The Rust tables are committed rather than built by a build script because a
> build script cannot read `database/` — it is above the crate — so the crates
> would not be publishable. The cost is that a bare `cargo build` does NOT pick
> up a database edit; use `make rust`, which runs `keel generate` first.
...
| `analyzer/pgn-generated-data.h`, `pgn-j1939-generated-data.h`, `fieldtype-generated-data.h`, `physicalquantity-generated-data.h` | **generated** | The analyzer's data tables, from `database/`. |
...
| `crates/canboat/src/engine/schema_generated.rs` | **generated** | `keel generate` output (~5.6 MB): the Rust PGN tables the `canboat` crate compiles in. Committed, not built — see §3. |
| `crates/canboat/src/io/fastpacket_generated.rs` | **generated** | `keel generate` output: fast-packet framing for PGNs 0x1F000..0x1FFFF. |
...
database/  (YAML — SOURCE OF TRUTH: pgns/, lookups/, fieldtypes.yaml, j1939/)
        ...
        │  keel generate
        ▼
   docs/canboat.xml (2.3 MB)  +  analyzer/lookup-generated-data.h + analyzer/*-data.h (C tables)
        │                        +  crates/canboat/src/engine/schema_generated.rs (Rust tables)
...
generated C headers** (`pgn-generated-data.h`, `lookup-generated-data.h`, …) — they are output.
...
| Add/change an **enum value** | `analyzer/lookup-generated-data.h` | same |
| Add a **field type / encoding** | `analyzer/fieldtype.h` (add type before use) | same |
| Add a **J1939 PGN** | `analyzer/pgn-j1939.h` | same |
| Add a **manufacturer code** | `analyzer/lookup-generated-data.h` (`MANUFACTURER_CODE`) | same (`util/update-manufacturer-codes.sh` drafts lines) |
