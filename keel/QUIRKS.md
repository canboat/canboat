...
| Q6 | Floats print as C `%g` (6 significant digits) / `%.15g`. Notably lossy for resolutions: 1/11 prints as `0.0909091`, so the XML/JSON never contains the exact wire resolution | all float printfs | `cformat.c_g()` / `c_15g()` | **keep** — downstream fixtures parse these exact strings; revisit only with a major schema bump |
