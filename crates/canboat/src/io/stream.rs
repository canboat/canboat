...
//! through [`crate::io::device::DeviceHandle`].
...
/// [`FrameSource`](crate::engine::FrameSource) trait, re-exported here
...
/// impl lives in the engine).
pub use crate::engine::FrameSource as FrameReader;
...
/// stdin lock, or the PLAIN [`BufRead`](crate::io::container::plain_reader)
/// that unwraps a `.nif` / `.pcap` capture container.
pub struct LineFrameReader<R: BufRead> {
    lines: crate::io::LineReader<R>,
    ...
    fn read_frame(&mut self) -> io::Result<Option<RawFrame>> {
        loop {
            ...
            match parse_with(format, line) {
                ...
                Ok(None) => continue,
                Err(crate::engine::format::PlainError::Empty) => continue,
                Err(e) => {
...
/// A device's [`FrameSender`](crate::io::device::FrameSender) is a
...
impl FrameWriter for crate::io::device::FrameSender {
    fn write_frame(&mut self, frame: &RawFrame) -> io::Result<()> {
