...
//! per-line parsing — lives in [`LineFrameReader`](crate::io::LineFrameReader).
...
pub fn decode_file<F: FnMut(&DecodedPgn)>(
    ...
    if crate::io::container::is_container(path) {
        let reader = crate::io::container::plain_reader(path, Default::default())
...
/// straight to `crate::engine::output::{write_json, write_text}`
/// without re-decoding.
///
...
";
    ...
    fn plain_line(f: &crate::engine::RawFrame) -> String {
    ...
    fn startup_record_is_suppressed_on_request() {
        let rec = crate::engine::startup::startup_record_at(
            "8.1.0",
            "canboat-test",
            "/dev/null",
