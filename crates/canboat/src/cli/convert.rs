...
//! to a [`RawFrame`](crate::engine::RawFrame) stream, then emitted as:
//!
//! - `plain` — canboat PLAIN/FAST lines (a pure frame→frame shunt via
//!   [`crate::io::copy`]); subsumes `nif2analyzer` and format
//!   normalisation.
...
/// The no-filter case is exactly [`crate::io::copy`]; with filters we
...
fn convert_decoded<W: Write>(
    ...
    let sink = |decoded: &crate::engine::DecodedPgn| {
        if sink_err.is_some() {
            return;
        }
        line.clear();
