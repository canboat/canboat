...
//! The `canboat` command-line tool: the subcommand modules, [`app`] (the
//! clap tree and dispatch that the binary's `main` forwards to), and the
//! plumbing they share. Compiled only with the `cli` feature; none of it
//! is part of the library's public surface.
//!
...
//! Each entry point calls `canboat_argv()` to normalise its argv before
//! handing it to `Cli::parse_from`.
//!
//! [`shape`] holds the decoded-output flags every command that emits
...
/// The canboat copyright line and schema version, generated by `keel`
/// from `database/`. Every command shows them in `--help` (via clap
/// `after_help`) and logs them at startup when verbose logging is on.
pub use crate::engine::{CANBOAT_JSON_VERSION, COPYRIGHT_ID};
