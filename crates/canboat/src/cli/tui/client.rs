...
//! drains the one-shot JSON blob, and seeds [`crate::cli::tui::state::AppState`]
...
//! [`crate::engine::snapshot::classify_json_line`] so the keys we
...
use crate::cli::tui::state::{AppState, Progress};
...
enum LoadItem {
    Frame(String),
    Record(crate::engine::snapshot::SnapshotInput),
}
...
fn ingest_raw_capture(path: &Path, tx: &mpsc::UnboundedSender<LoadItem>) -> anyhow::Result<()> {
    use crate::engine::format::write_plain;
    use crate::engine::frame::RawFrame;
    use crate::engine::output::{JsonOptions, write_json};
    use crate::engine::snapshot::classify_json_line;
    ...
    let cfg = crate::io::analyze::Config {
        units: crate::engine::Units::Metric,
        ..Default::default()
    };
    ...
    crate::io::analyze::decode_file(path, &cfg, |decoded| {
...
pub fn spawn_log_load(path: PathBuf, state: Arc<Mutex<AppState>>) -> Vec<JoinHandle<()>> {
    ...
    let decode = tokio::task::spawn_blocking(move || {
        let result = if crate::io::container::is_container(&path_for_blocker) {
...
async fn overrides_writer_task(
    ...
    loop {
        let mut line = tokio::select! {
            ...
            _ = poll.tick() => crate::cli::tui::iso::override_request(),
        };
        if !line.ends_with('\n') {
            line.push('\n');
        }
...
async fn filter_writer_task(
    ...
    loop {
        let mut line = tokio::select! {
            ...
            _ = poll.tick() => crate::cli::tui::iso::nmea0183_filter_request(),
        };
        if !line.ends_with('\n') {
            line.push('\n');
        }
...
async fn reader_task(reader: tokio::net::tcp::OwnedReadHalf, state: Arc<Mutex<AppState>>) {
    ...
    loop {
        ...
        crate::engine::snapshot::classify_json_line(trimmed, |i| inputs.push(i));
        if inputs.is_empty() {
            continue;
        }
        let mut s = state.lock().await;
...
mod tests {
    ...
    fn camel_snapshot_seeds_device_cache() {
        use crate::cli::tui::device_cache::NameKey;
        use crate::engine::snapshot::{SnapshotInput, SnapshotStore};
    ...
    fn writer_routes_by_pgn() {
        ...
        assert!(writer.send(crate::cli::tui::iso::nmea0183_filter_request()));
        assert!(writer.send(crate::cli::tui::iso::override_request()));
        assert!(writer.send(crate::cli::tui::iso::iso_request(35, 126464)));
