...
//!   [`crate::engine::snapshot::classify_json_line`] so the two paths
...
use crate::engine::{FieldRef, PgnDatabase, PgnInfo, Units, field};
use indexmap::IndexMap;
use serde_json::{Map, Value};

use crate::cli::tui::device_cache::{CachedInfo, NameKey};

/// `(pgn, src, secondary)` — same shape as
/// [`crate::engine::snapshot`]'s internal cache key.
...
impl AppState {
    ...
    pub fn upsert(
        ...
        let stamp_ms = timestamp.as_deref().and_then(crate::engine::parse_iso_ms);
...
mod tests {
    ...
    fn pgn_load_rate_is_count_over_capture_duration() {
        ...
        let mut s = state();
        let ts = |ms: i64| json!({"pgn": 1, "timestamp": crate::engine::format_iso_ms(ms as u64)});
