...
use crate::cli::tui::client::SaveFormat;
use crate::cli::tui::filebrowser::FileBrowser;
use crate::cli::tui::iso;
use crate::cli::tui::menu::{
...
impl App {
    ...
    pub fn detail_rows(&self, src: u8, state: &AppState) -> Vec<Entry> {
        ...
        // manufacturer (see [`crate::cli::tui::state::variant_for`]). Surface each
    ...
    fn handle_menu_key(
        ...
        key: KeyEvent,
        state: &AppState,
        writer: &crate::cli::tui::client::Writer,
    ) {
    ...
    fn activate_selection(&mut self, state: &AppState, writer: &crate::cli::tui::client::Writer) {
        match self.screen {
            Screen::Devices => {
    ...
    fn send_iso_126464(&mut self, src: u8, writer: &crate::cli::tui::client::Writer) {
    ...
    fn toggle_nmea0183_selection(
        ...
        if let Some(row) = self.nmea0183_state.selected().and_then(|i| rows.get(i)) {
            let sentence = row
                .sentence
                .as_deref()
                .unwrap_or(crate::cli::tui::state::NMEA0183_ALL);
    ...
    fn handle_modal_key(&mut self, key: KeyEvent, writer: &crate::cli::tui::client::Writer) {
...
fn robust_interval(e: &Entry, state: &AppState) -> Option<std::time::Duration> {
    ...
    let mut stamps: Vec<i64> = e
        ...
        .filter_map(crate::engine::parse_iso_ms)
        .collect();
    if stamps.len() < 2 {
        return e.interval();
    }
...
fn pgn_on_request(pgn: u32) -> bool {
    crate::engine::PgnDatabase::embedded(crate::engine::Units::Metric)
        .first_pgn(pgn)
...
fn variant_manufacturer(v: &crate::engine::PgnInfo) -> Option<u16> {
    v.fields
        .first()
...
fn variant_industry(v: &crate::engine::PgnInfo) -> Option<u8> {
    v.fields
        .get(2)
        .and_then(|f| f.match_value)
...
fn advertised_variant(
    pgn: u32,
    device_mfr: Option<u16>,
) -> Option<&'static crate::engine::PgnInfo> {
...
fn synthesize_advertised_entry(variant: &crate::engine::PgnInfo, src: u8) -> Entry {
    let now = std::time::Instant::now();
...
fn draw_pgn_lists(
    f: &mut ratatui::Frame<'_>,
    area: Rect,
    lists: &crate::cli::tui::state::PgnLists,
) {
    let tx = lists
        .tx
        .iter()
...
fn row_search_string(h: &crate::cli::tui::state::HistoryRecord) -> String {
...
fn format_time_row(idx: usize, h: &crate::cli::tui::state::HistoryRecord) -> Line<'static> {
...
mod tests {
    use super::*;
    use crate::cli::tui::state::Status;
    use ratatui::Terminal;
    ...
    fn robust_interval_ignores_merge_gaps() {
        ...
        let ts =
            |ms: i64| json!({"pgn": 126993, "timestamp": crate::engine::format_iso_ms(ms as u64)});
    ...
    fn open_dialog_swallows_menu_and_help_keys() {
        let state = seeded_state();
        let (writer, _rx) = crate::cli::tui::client::make_writer();
