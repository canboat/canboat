...
//! `canboat`: the single-binary front end to the CANboat toolkit. The
//! binary's `main` forwards straight to [`main`] here, so the subcommands
//! live inside the library where they can reach the private `engine` /
//! `io` / `server` modules that the public facade deliberately hides.
//!
...
use super::{convert, format_message, interface, legacy, replay, tui};
use crate::{n2kd, server};
...
#[command(
    ...
    version,
    after_help = crate::cli::help_footer()
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
