...
impl Bridge {
    ...
    /// address. A frame left at `src` 0 / [`ADDR_GLOBAL`](crate::engine::ADDR_GLOBAL)
...
impl Transmitter {
    ...
    /// [`ADDR_GLOBAL`](crate::engine::ADDR_GLOBAL) is stamped with the live
    ...
    pub fn transmit(&self, mut frame: RawFrame) -> Result<()> {
        if (frame.src == 0 || frame.src == crate::engine::ADDR_GLOBAL)
