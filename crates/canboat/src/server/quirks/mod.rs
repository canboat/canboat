...
//! decoded date in place, and lives in [`crate::engine::quirk`] so that
...
use crate::engine::quirk::Target;
use crate::engine::{DecodedPgn, RawFrame};
...
pub enum QuirkKind {
    ...
    /// Rewrites the decoded value (see [`crate::engine::quirk`]);
...
impl Quirks {
    pub fn new(kinds: Vec<QuirkKind>) -> Self {
        ...
        match QuirkKind::gps_rollover_target(&kinds) {
            Some(target) => crate::engine::quirk::enable_gps_rollover(target.clone()),
            None => crate::engine::quirk::disable_gps_rollover(),
        }
        Self {
    ...
    fn parses_every_name() {
        ...
        assert_eq!(
            ...
            Ok(QuirkKind::GpsRollover(Target::Devices(vec![
                crate::engine::quirk::Device::Address(4)
            ])))
        );
