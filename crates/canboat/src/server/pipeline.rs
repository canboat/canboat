...
use crate::engine::{FramePacketType, PgnDatabase, RawFrame, Reassembled, Reassembler};
...
pub struct Hubs {
    ...
    pub decoded_tx: Option<std::sync::mpsc::Sender<Arc<crate::engine::DecodedPgn>>>,
}
    ...
    loop {
        ...
        let assembled = if pre_coalesced.load(Ordering::Relaxed) {
            ...
            let packet_type = db
                ...
                .map(|p| match p.packet_type {
                    crate::engine::PacketType::Fast => FramePacketType::Fast,
                    crate::engine::PacketType::Single => FramePacketType::Single,
        ...
        if hubs.quirks.is_enabled() {
            ...
            let own_addr = hubs
                ...
                .filter(|&a| a != crate::engine::ADDR_GLOBAL && a != crate::engine::ADDR_NULL);
            for mut synth in hubs.quirks.process_decoded(&decoded, own_addr) {
                ...
                if (synth.src == 0 || synth.src == crate::engine::ADDR_GLOBAL)
        ...
        if let Some(ov) = hubs.overrides.as_ref() {
            if let Some(name) = decoded.iso_name() {
                ...
                use crate::engine::field::nmea_acknowledge_group_function as ack;
...
mod tests {
    ...
    fn tapped_hubs(decoded_tx: mpsc::Sender<Arc<crate::engine::DecodedPgn>>) -> Hubs {
    ...
    fn decoded_tap_forwards_each_record() {
        ...
        let claim =
            crate::engine::RawFrame::new(None, 6, 60928, 0x21, 0xFF, [1, 2, 3, 4, 5, 6, 7, 8]);
