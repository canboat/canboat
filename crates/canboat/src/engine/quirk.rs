...
//! `server::quirks` *emit* synthetic frames onto the bus; the ones here
...
mod tests {
    ...
    fn date_after(
        frame: &crate::engine::RawFrame,
        name: Option<u64>,
        target: &Target,
        reference_day: u16,
    ) -> Option<u16> {
        let _guard = SWITCH.lock().unwrap_or_else(|e| e.into_inner());
        let mut decoded = crate::engine::PgnDatabase::embedded(crate::engine::Units::Si)
            .decode(frame)
            .expect("decodes");
        apply_at(
            frame.pgn,
    ...
    /// was captured.
    fn dsc_call() -> crate::engine::RawFrame {
        let data: Vec<u8> =
            "74,6c,00,16,29,02,28,64,7e,39,30,30,30,31,36,ff,ff,ff,ff,ff,ff,02,01,ff,ff,ff,\
             ...
             ff,ff,40,f2,b5,17,d4,34,00,00"
                ...
                .collect();
        crate::engine::RawFrame::new(None, 4, 129808, 4, 255, data)
    }

    /// A PGN 129794 AIS Class A Static frame from `src` with an ETA
    /// date of `date`.
    fn ais_static(src: u8, date: u16) -> crate::engine::RawFrame {
        ...
        crate::engine::RawFrame::new(None, 6, 129794, src, 255, data)
    }
    ...
    fn the_decoder_learns_names_from_address_claims() {
        ...
        let date_of = |d: crate::engine::DecodedPgn| {
        ...
        let claim = crate::engine::RawFrame::new(None, 6, 60928, 4, 255, VHF_NAME.to_le_bytes());
        ...
        let claim = crate::engine::RawFrame::new(None, 6, 60928, 9, 255, VHF_NAME.to_le_bytes());
        db.decode(&claim).unwrap();
