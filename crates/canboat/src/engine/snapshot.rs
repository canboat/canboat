...
use crate::engine::startup::format_iso_ms;
...
/// Local overrides for PGNs whose schema definition is
...
/// path.
#[inline]
pub fn is_part_of_primary_key(pgn: u32, field: &crate::engine::FieldInfo) -> bool {
...
pub fn composite_secondary<F>(info: &crate::engine::PgnInfo, mut get_text: F) -> Option<String>
where
    F: FnMut(&crate::engine::FieldInfo) -> Option<String>,
{
...
pub fn repeating_pk_set(info: &crate::engine::PgnInfo) -> Option<u8> {
    for f in info.fields {
        if !is_part_of_primary_key(info.pgn, f) {
            continue;
        }
...
where
    F: FnMut(&crate::engine::FieldInfo) -> Option<String>,
    G: FnMut(&crate::engine::FieldInfo, u32) -> Option<String>,
{
    let mut parts: Vec<String> = Vec::new();
    for f in info.fields {
        if !is_part_of_primary_key(info.pgn, f) {
...
/// size)`.
fn field_in_any_repeat_set(
    pgn: &crate::engine::PgnInfo,
    field: &crate::engine::FieldInfo,
) -> Option<u8> {
...
{
    let Some(pgn) = crate::engine::analyzer_json::int(line, "pgn") else {
        return 0;
    };
    let pgn = pgn as u32;
    let Some(src) = crate::engine::analyzer_json::int(line, "src") else {
        return 0;
    };
    let src = src as u8;
    let description = crate::engine::analyzer_json::value(line, "description")
        .unwrap_or("")
        .to_string();
    let is_ais = is_ais_pgn(pgn);
    let info = crate::engine::PgnDatabase::embedded(crate::engine::Units::Metric).first_pgn(pgn);
    ...
    let use_camel = crate::engine::analyzer_json::camel_wrapper_id(line).is_some()
        || info.is_some_and(|i| crate::engine::analyzer_json::uses_camel_keys(line, i.fields));
    let field_key = move |f: &crate::engine::FieldInfo| if use_camel { f.id } else { f.name };
    ...
    let Some(rs) = repeat_set else {
        let secondary = info.and_then(|info| {
            composite_secondary(info, |f| {
                crate::engine::analyzer_json::lookup_text(line, field_key(f)).map(|s| s.to_string())
            })
        });
        sink(SnapshotInput {
            pgn,
    ...
    let Some((arr_start, arr_end)) = crate::engine::analyzer_json::array_span(line, list_key)
    else {
    ...
    let elements = crate::engine::analyzer_json::split_object_array(arr);
    ...
    for (iter, elem) in elements.iter().enumerate() {
        let secondary = per_iteration_secondary(
            ...
            |f, _| {
                crate::engine::analyzer_json::lookup_text(elem, field_key(f)).map(|s| s.to_string())
            },
        );
...
impl SnapshotStore {
        ...
        F: Fn(&u32, &CacheEntry) -> bool,
    {
        ...
        let db = crate::engine::PgnDatabase::embedded(crate::engine::Units::Metric);
        ...
        for (pgn, entries) in by_pgn.iter() {
            ...
            let camel_id = crate::engine::analyzer_json::camel_wrapper_id(&rendered[0]);
...
mod tests {
    ...
    fn repeating_pk_set_detects_pgn_130824() {
        ...
        let info = crate::engine::PgnDatabase::embedded(crate::engine::Units::Metric)
            .first_pgn(130824)
