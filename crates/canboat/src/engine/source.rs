...
//! [`crate::io`]. [`Decoder`] wraps a source with fast-packet reassembly and
//! the schema, yielding a stream of [`DecodedPgn`]s.
//!
//! This is the one place the engine names [`std::io::Result`]: the trait
//! only *describes* a read and performs no I/O itself. The concrete readers
//! (serial, file, `.pcap`/`.nif` container) live in [`crate::io`].
...
///
/// ```no_run
/// use canboat::{Database, Units, read::Decoder};
/// # fn go<S: canboat::read::FrameSource>(source: S) {
...
impl<S: FrameSource> Decoder<S> {
    ...
    pub fn units(&self) -> crate::engine::Units {
        self.db.units()
    }
}
