...
use crate::engine::bits::{Extracted, Sentinel, classify_sentinel, extract_bits};
use crate::engine::db::PgnDatabase;
use crate::engine::frame::RawFrame;
use crate::engine::types::{FieldInfo, FieldType, PgnInfo};
...
impl DecodedField {
    ...
    /// [`crate::engine::units::convert_unit`]). A unitless field is returned
    ...
    /// matter which schema ([`crate::engine::Units`]) produced the field — e.g.
    ...
    pub fn as_f64_in(&self, target: &str) -> Option<f64> {
        ...
        match self.unit() {
            Some(from) => crate::engine::units::convert_unit(v, from, target),
            None => Some(v),
        }
    }
...
pub struct DecodedPgn {
    ...
    pub id: &'static str,
    /// Mirror of [`crate::engine::types::PgnInfo::id_is_pinned`]: `true` when
...
impl DecodedPgn {
    /// Look up a non-repeating field by [`crate::engine::FieldRef`] — a generated
    /// constant (`crate::engine::field::wind_data::WIND_ANGLE`) or one
    /// resolved at runtime via [`crate::engine::PgnDatabase::field`]. Returns
    ...
    #[inline]
    pub fn field(&self, f: crate::engine::FieldRef) -> Option<&DecodedField> {
        debug_assert_eq!(
            f.pgn.id, self.id,
    ...
    /// [`crate::engine::FieldRef`] constant (e.g. AIS encoders that
...
impl PgnDatabase {
    ...
    pub fn decode(&self, frame: &RawFrame) -> Result<DecodedPgn, DecodeError> {
        ...
        if frame.pgn == 60928 {
            crate::engine::quirk::note_address_claim(frame.src, &frame.data);
        }
        crate::engine::quirk::apply(frame.pgn, frame.src, &mut fields);
    ...
    pub fn pick_variant(&self, frame: &RawFrame) -> Option<&'static PgnInfo> {
        // Generated by keel: a `match` jumps to a per-PGN dispatch
        ...
        // sparse cross-PGN catch-all when no entry for this PGN
        // number exists at all.
        // Each schema flavor (NMEA 2000, J1939) carries its own
        // dispatch pair; the returned index is only valid against this
        // db's own PGN arrays (identical across SI and Metric), so map
        // it through `pgn_slice` for the right units.
        let idx = (self.dispatch)(frame.pgn, &frame.data).or_else(|| (self.catchall)(frame.pgn))?;
...
struct DecodeContext {
    ...
    dynamic_field_type: Option<crate::engine::types::LookupFieldTypeValue>,
...
fn decode_text(v: &[u8], encoding: Option<&str>) -> String {
    match std::str::from_utf8(v) {
        ...
        Err(_) => match encoding {
            Some("RDS_G0") => v.iter().map(|&b| crate::engine::rds_g0_char(b)).collect(),
...
fn decode_dynamic_number(
    ...
    bit_length: u32,
    signed: bool,
    entry: &crate::engine::types::LookupFieldTypeValue,
) -> FieldValue {
...
mod tests {
    ...
    fn bitlookup_keeps_set_bits_the_enumeration_does_not_name() {
        ...
        crate::engine::output::write_json(
            &mut json,
            &dec,
            &crate::engine::output::JsonOptions {
                name_value: true,
                ..Default::default()
            },
        )
    ...
    fn truncated_trailing_reserved_is_omitted() {
        ...
        crate::engine::output::write_json(
            &mut json,
            &dec,
            &crate::engine::output::JsonOptions::default(),
        )
        .unwrap();
    ...
    #[test]
    fn j1939_flavor_resolves_ecu1_where_main_falls_back() {
        // PGN 61444 (EEC1, engine speed) is a J1939 definition: the
        // J1939 database must resolve it by id, while the main NMEA
        // 2000 database only has the 0xF000-0xFEFF range catch-all for
        // that number.
        let frame = RawFrame {
            timestamp: None,
            prio: 3,
            ...
            src: 0,
            dst: 255,
            data: smallvec::smallvec![0xff, 0xff, 0xff, 0xe0, 0x2e, 0xff, 0xff, 0xff],
        };
        let j1939 = crate::engine::db::PgnDatabase::embedded_j1939(crate::engine::db::Units::Si);
        let picked = j1939.pick_variant(&frame).expect("j1939 variant");
        assert_eq!(picked.id, "ecu1", "got id={}", picked.id);

        let main = db().pick_variant(&frame).expect("main variant");
        assert!(
            main.fallback.unwrap_or(false),
            "main db must fall back for 61444, got id={}",
            main.id
        );
    }

    #[test]
    fn j1939_flavor_keeps_shared_iso_pgns() {
        // The ISO PGNs both buses share must resolve in the J1939
        // flavor too — address claim is the canary.
        let frame = RawFrame {
            timestamp: None,
            prio: 6,
            pgn: 60928,
        ...
        let j1939 = crate::engine::db::PgnDatabase::embedded_j1939(crate::engine::db::Units::Si);
        let picked = j1939.pick_variant(&frame).expect("j1939 variant");
        assert_eq!(picked.id, "isoAddressClaim", "got id={}", picked.id);
    }

    #[test]
    fn j1939_flavor_does_not_know_nmea2000_pgns() {
        // Exclusive-table semantics (matches keel decode and the C
        // analyzer / analyzer-j1939 pair): an NMEA-2000-only PGN must
        // not resolve to a specific definition in the J1939 flavor.
        let frame = RawFrame {
            timestamp: None,
            prio: 2,
            pgn: 129025, // Position, Rapid Update — pure NMEA 2000
            src: 0,
            dst: 255,
            data: smallvec::smallvec![0x00; 8],
        };
        let j1939 = crate::engine::db::PgnDatabase::embedded_j1939(crate::engine::db::Units::Si);
        let picked = j1939.pick_variant(&frame);
        assert!(
            picked.is_none_or(|p| p.fallback.unwrap_or(false)),
            "129025 must not resolve specifically in the J1939 flavor, got {:?}",
            picked.map(|p| p.id)
        );
    }
