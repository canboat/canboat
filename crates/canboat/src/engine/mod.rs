...
//! The engine: the sans-I/O NMEA 2000 (CANboat) PGN database, parsers,
//! decoder, encoder and output formatters. Named `engine` rather than
//! `core` because a `mod core` would shadow the `core` crate.
//!
//! Nothing in here does I/O. All functions are sync. Bytes go in, events
//! and bytes come out. The caller drives the I/O — see [`crate::io`] for
//! the sync adapters. The sole `std::io` touchpoint is the
//! [`FrameSource`] trait (the bring-your-own-transport seam): it names
//! an `io::Result` but performs no I/O itself.
//!
//! This module is private; the `canboat` crate root re-exports the
//! supported subset under its locked public names.
...
mod smoke {
    ...
    fn si_and_metric_schemas_decode_same_frame_in_their_units() {
        ...
        let si = angle(crate::engine::Units::Si);
        let metric = angle(crate::engine::Units::Metric);
