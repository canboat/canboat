...
//! inverse of [`crate::engine::decode`].
//!
...
//! offsets — LSB-first, exactly the layout [`crate::engine::bits::extract_bits`]
//! reads back — and returns a [`RawFrame`]. Because the result is a
//! `RawFrame`, any [`crate::engine::format`] writer (PLAIN, YDWG02, Actisense,
...
/// caller-constructible (unlike the decode [`crate::engine::FieldValue`], which
...
impl PgnBuilder {
    ...
    /// (`crate::engine::field::wind_data::WIND_ANGLE`) — the compile-checked,
    /// `O(1)` path, and the recommended default. The exact mirror of the
    /// decode side's [`DecodedPgn::field`](crate::engine::DecodedPgn::field): index
    ...
    /// [`DecodedPgn::field_by_name`](crate::engine::DecodedPgn::field_by_name).
    ...
    fn match_target_variant(&self, target_pgn: u32) -> Option<&'static PgnInfo> {
        ...
        if crate::engine::decode::is_pgn_proprietary(target_pgn)
...
/// — the exact layout [`crate::engine::bits::extract_bits`] reads back.
...
mod tests {
    ...
    fn iso_request_matches_c_format_message() {
        ...
        let frame = db()
            .encode_for(crate::engine::pgn::ISO_REQUEST)
            .priority(6)
            .destination(255)
            .push(crate::engine::field::iso_request::PGN, 126996u32)
            .unwrap()
            .build()
            .unwrap();
    ...
    fn unset_fields_default_and_length_checks() {
        ...
        let frame = db().encode_for(crate::engine::pgn::ISO_REQUEST).build();
    ...
    fn wind_data_round_trips_through_decode() {
        ...
        use crate::engine::field::wind_data as wd;
        let db = db();
        let frame = db
        ...
        let num = |name: &str| match &decoded.field_by_name(name).unwrap().value {
            crate::engine::FieldValue::Number(x) => *x,
            crate::engine::FieldValue::Integer(n) => *n as f64,
        ...
        match &decoded.field_by_name("Reference").unwrap().value {
            crate::engine::FieldValue::Lookup { value, name } => {
                assert_eq!(*value, 2);
    ...
    fn group_function_variable_values_encode_via_the_referenced_pgn() {
        ...
        match &val.value {
            crate::engine::FieldValue::Number(x) => assert!((x - 1.5).abs() < 0.01, "got {x}"),
            crate::engine::FieldValue::Integer(n) => {
                assert!((*n as f64 - 1.5).abs() < 1.0, "got {n}")
            }
    ...
    fn variable_target_variant_follows_the_parameter_pairs() {
        ...
        let val = decoded
            ...
            .expect("decoded VARIABLE value");
        match &val.value {
            crate::engine::FieldValue::Number(x) => assert!((x - 3.21).abs() < 0.005, "got {x}"),
            other => panic!("value: {other:?}"),
        }
    }
    ...
    fn string_lz_round_trips() {
        use crate::engine::field::navico_ascii_identifier::IDENTIFIER;
        let frame = db()
    ...
    fn binary_round_trips() {
        ...
        use crate::engine::field::iso_transport_protocol_data_transfer::DATA;
        ...
        match d.field(DATA).map(|f| &f.value) {
            Some(crate::engine::FieldValue::Binary(v)) => assert_eq!(v, &payload),
    ...
    fn float_round_trips() {
        ...
        use crate::engine::field::garmin_autopilot_heading_to_steer::HEADING_TO_STEER as HTS;
        let frame = db()
    ...
    fn iso_name_round_trips() {
        ...
        match d.field(SOURCE).map(|f| &f.value) {
            Some(crate::engine::FieldValue::IsoName { value, .. }) => assert_eq!(*value, name),
    ...
    fn zero_instance_repeating_pgn_builds_with_zero_count() {
        ...
        match &decoded
            ...
            .value
        {
            crate::engine::FieldValue::Integer(n) => assert_eq!(*n, 0),
