...
pub struct TextOptions {
    ...
    // TODO: the text formatter never consults this, so `-empty` is a
    // no-op for text output. Surfaced when the crates were folded into
    // one and the field stopped being public API.
    #[allow(dead_code)]
...
pub fn write_text<W: fmt::Write>(w: &mut W, pgn: &DecodedPgn, opts: &TextOptions) -> fmt::Result {
    ...
    if let Some(ts) = &pgn.timestamp {
        w.write_str(&crate::engine::format::normalize_timestamp(ts))?;
        w.write_char(' ')?;
    }
    write!(
        w,
...
fn write_text_debug_suffix<W: fmt::Write>(
    ...
    use crate::engine::bits::extract_bits;
    let signed = matches!(
        f.value,
...
mod tests {
    ...
    fn end_to_end_iso_address_claim() {
        ...
        // expected shape.
        use crate::engine::{PgnDatabase, RawFrame};
