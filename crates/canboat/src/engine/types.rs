...
//!
//! This module carries no data of its own. The full database is emitted
//! by `keel generate` from `database/` into the `&'static` tables of
//! [`schema_generated`](super::schema_generated) (and its J1939 twin),
//! which use these types.
//!
...
///
/// `keel generate` emits one `static` of this type per
/// (PGN, field) under `crate::engine::field::<pgn_id>::<FIELD_ID>`, so
...
pub struct LookupFieldTypeTable {
    pub name: &'static str,
    /// Emitted by keel for symmetry with the other tables; nothing
    /// reads it at decode time yet.
    #[allow(dead_code)]
    pub max_value: Option<u64>,
