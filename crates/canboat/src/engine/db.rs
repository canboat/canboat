...
impl PgnDatabase {
    ...
    /// [`crate::engine::decode::PgnDatabase::pick_variant`]).
    ...
    /// `"isoRequest"`, `"windData"`). See [`crate::engine::encode`].
    pub fn encode(
        &'static self,
        pgn_id: &str,
    ) -> Result<crate::engine::encode::PgnBuilder, crate::engine::encode::EncodeError> {
        let pgn = self
            .pgn_by_id(pgn_id)
            .ok_or_else(|| crate::engine::encode::EncodeError::NoSuchPgnId(pgn_id.to_string()))?;
        Ok(crate::engine::encode::PgnBuilder::for_pgn(self, pgn))
    }

    /// Start encoding a message for a generated PGN constant
    /// (`crate::engine::pgn::WIND_DATA`) — the compile-checked counterpart
    ...
    pub fn encode_for(&'static self, pgn: &'static PgnInfo) -> crate::engine::encode::PgnBuilder {
        let resolved = self.pgn_by_id(pgn.id).unwrap_or(pgn);
        crate::engine::encode::PgnBuilder::for_pgn(self, resolved)
    }

    /// Start encoding a message by PGN number. Returns
    /// [`crate::engine::encode::EncodeError::AmbiguousPgn`] when the number has
    ...
    pub fn encode_by_pgn(
        ...
        let first = variants
            .next()
            .ok_or(crate::engine::encode::EncodeError::NoSuchPgn(pgn))?;
        let extra = variants.count();
        if extra > 0 {
            return Err(crate::engine::encode::EncodeError::AmbiguousPgn {
                pgn,
                variants: extra + 1,
            });
        }
        Ok(crate::engine::encode::PgnBuilder::for_pgn(self, first))
    }

    /// Resolve a (PGN id, field id) pair into a [`FieldRef`] that later
    /// indexes into [`crate::engine::DecodedPgn`] at `O(1)` via
    /// [`crate::engine::DecodedPgn::field`]. Both inputs are canboat-json `Id`
