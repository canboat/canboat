...
//!
//! [`Codec::open`] writes only step 1; the rest flow back as
//! [`Event::Send`] each time the codec sees an ACK. [`Codec::close`]
//! sends `N2NET_OFFLINE` again, taking the gateway off the bus.
//!
...
pub struct Config {
    ...
    /// NOT ON IT IS REFUSED: THE CODEC DOES NOT SEND IT.** The gateway
...
impl Ikonvert {
    ...
    pub fn new(config: Config) -> Self {
        let init_state = if config.skip_init {
            STATE_DONE
        } else {
            // After `open()` sends `N2NET_OFFLINE`, the codec waits for
            // that ACK — start at the corresponding odd state.
...
impl Codec for Ikonvert {
            ...
            match ikonvert::parse_line(trimmed) {
                ...
                Err(e) => {
                    ...
                    if let Some(idx) = trimmed.find("$PDGY,TEXT,") {
                        ...
                        self.handle_control(banner, now_ms, events);
                    } else {
                        events.push(Event::Error(format!("ikonvert: {e}")));
                    }
                }
            }
        }
    ...
    fn send(&mut self, frame: &RawFrame) -> Result<Vec<u8>, Refused> {
        ...
        Ok(ikonvert::encode_tx_frame(frame).into_bytes())
    }
}

impl Ikonvert {
    fn handle_control(&mut self, body: &str, now_ms: u64, events: &mut Vec<Event>) {
    ...
    fn advance_after_text(&mut self, events: &mut Vec<Event>) {
        match self.init_state {
            STATE_WAIT_OFFLINE_ACK => {
                self.init_state = STATE_SEND_RESET;
                if let Some(cmd) = next_init_command(&mut self.init_state, &self.init) {
                    events.push(Event::Send(cmd.into_bytes()));
                } else {
    ...
    fn advance_after_ack(&mut self, events: &mut Vec<Event>) {
    ...
    fn maybe_resend_offline(&self, events: &mut Vec<Event>) {
        if self.init_state != STATE_WAIT_OFFLINE_ACK {
            return;
        }
        log::debug!("ikonvert: resending N2NET_OFFLINE");
        events.push(Event::Send(TX_OFFLINE.as_bytes().to_vec()));
    }
}
...
mod tests {
    ...
    /// first command via `open`, then feed the first response
    ...
    fn run_init(config: Config) -> Vec<String> {
        ...
        let first = String::from_utf8(codec.open()).unwrap();
        ...
        loop {
            ...
            if first_response {
                codec.handle_control("TEXT,iKonvert_simulated", NOW, &mut events);
                first_response = false;
            } else {
                codec.handle_control("ACK,test", NOW, &mut events);
            }
            let mut emitted_one = false;
            for ev in events {
                if let Event::Send(b) = ev {
    ...
    /// Frames carry the caller's time, not the gateway's seconds-since-boot
    /// counter, and a line split across reads is held until it completes.
    #[test]
    fn frames_are_stamped_with_the_callers_time() {
        ...
        let mut events = Vec::new();
        codec.receive(b"!PDGY,127257,3,35,255,12.3", NOW, &mut events);
        assert!(events.is_empty(), "half a line must not emit");
        codec.receive(b"45,AQID\r\n", NOW, &mut events);
    ...
    fn encoder_for(config: &Config) -> Ikonvert {
        Ikonvert::new(config.clone())
    }
    ...
    fn a_named_transmit_list_refuses_the_rest() {
        let mut encoder = encoder_for(&Config {
        ...
        for pgn in [127508, 130306, 127258, 59904, 126996] {
            assert!(encoder.send(&frame(pgn)).is_ok(), "{pgn} allowed");
        }
        assert_eq!(
            encoder.send(&frame(127506)),
            Err(Refused::NotOnTxList),
            "not listed"
        );
        assert_eq!(encoder.send(&frame(0x40100)), Err(Refused::Synthetic));
    }
    ...
    fn without_a_named_list_nothing_is_refused() {
        for config in [
            ...
            assert!(encoder_for(&config).send(&frame(127506)).is_ok());
        }
    }
    ...
    fn extra_ack_after_done_is_ignored() {
        let mut decoder = Ikonvert::new(Config::default());
        let mut events = Vec::new();
        decoder.handle_control("TEXT,welcome", NOW, &mut events); // → RESET
        events.clear();
        decoder.handle_control("ACK,reset", NOW, &mut events); // → INIT,ALL
        events.clear();
        decoder.handle_control("ACK,init", NOW, &mut events); // → done
        events.clear();
        decoder.handle_control("ACK,stray", NOW, &mut events); // ignored
    ...
    fn closing_sends_offline() {
        let mut live = Ikonvert::new(Config::default());
        assert_eq!(live.close(), b"$PDGY,N2NET_OFFLINE\r\n");
        let mut replay = Ikonvert::new(Config {
            skip_init: true,
            ..Config::default()
        });
        assert!(replay.open().is_empty());
        assert!(replay.close().is_empty());
    }
    ...
    fn heartbeat_at_offline_wait_resends_offline() {
        ...
        let mut decoder = Ikonvert::new(Config::default());
        let mut events = Vec::new();
        decoder.handle_control("000000,,,,,,", NOW, &mut events);
    ...
    fn populated_heartbeat_emits_synthetic_frame() {
        ...
        decoder.handle_control("000000,38,1,38,753,2,0", NOW, &mut events);
    ...
    fn text_after_init_triggers_reinit() {
        let mut decoder = Ikonvert::new(Config::default());
        let mut events = Vec::new();
        decoder.handle_control("TEXT,banner", NOW, &mut events); // → RESET
        events.clear();
        decoder.handle_control("ACK,reset", NOW, &mut events); // → INIT,ALL
        events.clear();
        decoder.handle_control("ACK,init", NOW, &mut events); // → done
        ...
        decoder.handle_control("TEXT,banner2", NOW, &mut events);
    ...
    fn corrupt_line_with_embedded_text_still_resets() {
        ...
        let mut events = Vec::new();
        decoder.handle_control("TEXT,init", NOW, &mut events); // → RESET
        events.clear();
        decoder.handle_control("ACK,reset", NOW, &mut events); // → INIT,ALL
        events.clear();
        decoder.handle_control("ACK,init", NOW, &mut events); // → done
