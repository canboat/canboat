...
//!
//! [`convert_decoded`] takes an already-decoded [`crate::engine::DecodedPgn`]
...
pub fn convert(out: &mut String, msg: &str, rate_limiter: &mut RateLimiter) -> usize {
    let Some(decoded) =
        crate::engine::json_to_decoded(msg, PgnDatabase::embedded(crate::engine::Units::Metric))
    else {
        return 0;
    };
...
mod tests {
    ...
    fn mwv_converts_from_si_radians() {
        ...
        let decoded =
            crate::engine::json_to_decoded(msg, PgnDatabase::embedded(crate::engine::Units::Si))
                .expect("decode");
        let mut out = String::new();
        let mut rl = RateLimiter::new(false);
