...
fn run_stdin_pump(hub: &Hub) -> Result<()> {
    ...
    loop {
        ...
        let units = if unit_si {
            crate::engine::Units::Si
        } else {
            crate::engine::Units::Metric
        };
        let decoded = crate::engine::json_to_decoded(trimmed, PgnDatabase::embedded(units));
...
/// [`crate::engine::snapshot::classify_json_line`], which the hub
...
impl Hub {
    ...
    fn store(&self, line: &str) {
        crate::engine::snapshot::classify_json_line(line, |input| self.cache.store(input));
    }
...
mod tests {
    ...
    fn classify_secondary(line: &str) -> Option<String> {
        let mut got = Vec::new();
        crate::engine::snapshot::classify_json_line(line, |i| got.push(i));
    ...
    fn ais_message_marks_is_ais_and_keys_on_user_id() {
        ...
        assert!(crate::engine::snapshot::is_ais_pgn(129039));
