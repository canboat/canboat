...
use canboat::{Database, FieldValue, Frame};
...
fn decode_iso_address_claim() -> canboat::DecodedPgn {
    let frame = Frame {
    ...
    Database::embedded(canboat::Units::Metric)
        .decode(&frame)
        .expect("decode")
}
...
fn field_handle_indexed_access() {
    ...
    let db = Database::embedded(canboat::Units::Metric);
